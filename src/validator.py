...
validator.py
------------
独立校验模块：在优化结果确定容量后，使用全年 8760 小时完整数据
进行时序生产模拟，验证系统的实际运行指标（弃电率、缺电率等）。
"""
...
class SystemValidator:
    ...
    def validate(self, capacities: Dict[str, float]) -> Dict[str, Any]:
        ...
        try:
            status = n.optimize(solver_name=solver, include_objective_constant=False)
        except RuntimeError as e:
            logger.warning(f"Validation solver {solver} failed ({e}), trying automatic selection...")
            status = n.optimize(include_objective_constant=False)

        if not is_solver_optimal(status):
            raise RuntimeError(f"Validation dispatch optimization failed. Status: {status}")
    ...
    def _add_components(self, n: pypsa.Network, capacities: Dict[str, float]):
        ...
        fuel_cost = self.config["costs"]["thermal"]["fuel_cost"] * 1000
    ...
    def analyze_results(self, n: pypsa.Network, capacities: Dict[str, float]) -> Dict[str, Any]:
        ...
        return {
            "shed_hours": int(shed_hours),
            "total_shed_mwh": float(total_shed),
            "max_shed_mw": float(max_shed),
            "shed_events": shed_events,
            "curtailment_rate": float(curtailment_rate),
            "total_gen_mwh": float(total_gen),
            "total_avail_mwh": float(total_avail)
        }

    def validate_constraints(self, val_results: Dict[str, Any]) -> None:
        """校验关键约束是否满足。"""
        tx = self.config["constraints"]["transmission"]
        max_curtailment = tx.get("max_curtailment_rate", 0.10)
        max_shed_rate = tx.get("max_load_shedding_rate", 0.0)
        strict = self.config.get("settings", {}).get("strict_validation", False)

        errors = []

        if val_results["curtailment_rate"] > max_curtailment + 1e-9:
            errors.append(
                f"弃电率 {val_results['curtailment_rate']*100:.2f}% 超过配置上限 {max_curtailment*100:.2f}%"
            )

        if max_shed_rate > 0:
            total_load = self.full_data["load_mw"].sum()
            if val_results["total_shed_mwh"] > max_shed_rate * total_load + 1e-6:
                errors.append(
                    f"缺电量 {val_results['total_shed_mwh']:.2f} MWh 超过上限 {max_shed_rate * total_load:.2f} MWh"
                )
        ...
        if errors:
            for err in errors:
                if strict:
                    ...
                    logger.warning(err)
            if strict:
                raise ConstraintViolationError("; ".join(errors))

        if not errors:
            logger.info("All validation constraints satisfied.")
        else:
            logger.info("Validation constraints violated, but strict_validation is disabled.")
